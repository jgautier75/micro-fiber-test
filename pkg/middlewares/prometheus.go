...
// PrometheusNew creates a new instance of FiberPrometheus middleware
// serviceName is available as a const label
func PrometheusNew(serviceName string) *FiberPrometheus {
	return create(prometheus.DefaultRegisterer, serviceName, "http", "", nil)
}

// PrometheusNewWith creates a new instance of FiberPrometheus middleware but with an ability
...
func PrometheusNewWith(serviceName, namespace, subsystem string) *FiberPrometheus {
	return create(prometheus.DefaultRegisterer, serviceName, namespace, subsystem, nil)
}

// PrometheusNewWithLabels creates a new instance of FiberPrometheus middleware but with an ability
// to pass namespace and a custom subsystem
...
func PrometheusNewWithLabels(labels map[string]string, namespace, subsystem string) *FiberPrometheus {
	return create(prometheus.DefaultRegisterer, "", namespace, subsystem, labels)
}

// PrometheusNewWithRegistry creates a new instance of FiberPrometheus middleware but with an ability
...
// then then metrics would become
// `my_app_http_requests_total{...,key1= "value1", key2= "value2" }“
func PrometheusNewWithRegistry(registry prometheus.Registerer, serviceName, namespace, subsystem string, labels map[string]string) *FiberPrometheus {
